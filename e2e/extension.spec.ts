...
async function addHeader(popupPage: Page, headerName: string, headerValue: string, scope?: string) {
...
test.describe('mirrord browser extension', () => {
  test('popup shows inactive state on fresh install', async ({ popupPage }) => {
    await expect(popupPage.getByText('Inactive')).toBeVisible()
  })

  test('add a header and verify it appears in popup', async ({ popupPage }) => {
    await addHeader(popupPage, 'X-Mirrord-Test', 'test-value-123')

    await expect(popupPage.getByText('Active', { exact: true })).toBeVisible()
    await expect(popupPage.getByText('X-Mirrord-Test: test-value-123')).toBeVisible()
    await expect(popupPage.getByText('All URLs')).toBeVisible()
  })

  test('header is injected into real HTTP requests', async ({ context, popupPage }) => {
    await addHeader(popupPage, 'X-Mirrord-Test', 'test-value-123')
  ...
  test('scoped header only injected on matching URLs', async ({ context, popupPage }) => {
    await addHeader(popupPage, 'X-Scoped-Header', 'scoped-value', '*://localhost:3456/scoped/*')
  ...
  test('header is injected into asset requests (scripts, stylesheets, images)', async ({
    ...
    const assetHeaders = JSON.parse(body) as Record<string, HeadersMap | undefined>

    // Verify header was injected into script requests
    expect(assetHeaders['script.js']?.['x-mirrord-test']).toBe('asset-injection')

    // Verify header was injected into stylesheet requests
    expect(assetHeaders['style.css']?.['x-mirrord-test']).toBe('asset-injection')

    // Verify header was injected into image requests
    expect(assetHeaders['logo.png']?.['x-mirrord-test']).toBe('asset-injection')
  })
  ...
  test('reset to defaults restores CLI-provided config', async ({
    ...
    await expect(popupPage.getByText('X-Override: override-val')).toBeVisible()
    ...
    // Reset to defaults
    await expect(popupPage.getByRole('button', { name: 'Reset to Default' })).toBeVisible()
    await popupPage.getByRole('button', { name: 'Reset to Default' }).click()
    await expect(popupPage.getByText('Reset!')).toBeVisible()

    // Verify default header is now active
    await expect(popupPage.getByText('X-Default-Header: default-val')).toBeVisible()

...
test('session monitor authenticates and auto-configures the extension', async ({
  ...
  await expect
    .poll(async () =>
      extension.evaluate(async () => {
        const stored = await chrome.storage.local.get(['mirrord_ui_backend', 'mirrord_ui_token'])
  ...
  const operatorResponse = await fetch(`${REAL_BACKEND}/api/v2/operator/sessions`, {
    headers: { 'x-auth-token': TOKEN },
  })
  const operatorAvailable =
    operatorResponse.ok &&
    ((await operatorResponse.json()) as LiveOperatorSessions).status === 'available'
  ...
  await expect(extension.getByText('mirrord ui token rejected')).not.toBeVisible()
})

test('monitor Join reaches the extension through the bridge', async ({ context, extensionId }) => {
  test.skip(!TOKEN, 'MIRRORD_UI_TOKEN env var required')

  const monitor = await context.newPage()
  await monitor.goto(`${REAL_BACKEND}/auth?token=${encodeURIComponent(TOKEN)}&redirect=/`)
  await expect(monitor).toHaveURL(`${REAL_BACKEND}/`)
  ...
  await expect
            ...
            new Promise<string[]>((resolve) => {
              chrome.declarativeNetRequest.getDynamicRules((rules) =>
                resolve(
                  rules.flatMap(
                    (r) =>
                      r.action.requestHeaders?.map((h) => `${h.header}=${h.value ?? ''}`) ?? [],
                  ),
                ),
              )
            }),
    ...
    .toEqual(expect.arrayContaining([expect.stringContaining('mirrord-session=')]))
})

test('live join against a visible operator session', async ({ context, extensionId }) => {
  test.skip(!TOKEN, 'MIRRORD_UI_TOKEN env var required')
  ...
  const operatorData = operatorResponse.ok
    ...
    : null
  test.skip(
    operatorData?.status !== 'available' || operatorData.sessions.length === 0,
  ...
  const anyJoinButton = popup.getByRole('button', { name: /^join /i }).first()
  ...
  const dnrRules = await popup.evaluate(async () => {
    return await new Promise<chrome.declarativeNetRequest.Rule[]>((resolve) => {
  ...
  expect(headerOp?.header.toLowerCase()).toMatch(/baggage|x-mirrord-user|mirrord/i)

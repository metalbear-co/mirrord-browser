...
export function SessionsView({
  ...
  const filtered = useMemo(() => {
    return sessions.filter(
      (s) => (!namespace || s.namespace === namespace) && matchesQuery(s, normalizedQuery),
    )
  ...
  const joinedLive = joinedKey !== null && sessions.some((s) => s.key === joinedKey)
  const liveness = useJoinLiveness(joinedKey !== null, joinedLive, JOIN_GRACE_MS)
  ...
  if (!sessionsLoaded) {
    return uiDetectedNoToken ? <MirrordUiDetectedPrompt /> : <NotConfiguredPrompt />
  }
  ...
  const joinedSessions = joinedKey ? sessions.filter((s) => s.key === joinedKey) : []
  ...
  const VISIBLE_CAP = 5
  const visibleKeys = showAll || normalizedQuery ? orderedKeys : orderedKeys.slice(0, VISIBLE_CAP)
  ...
  return (
    <div className="flex flex-col" style={{ gap: 10 }}>
      ...
      {(contexts.length > 1 || namespaces.filter((ns) => ns !== '').length > 1) && (
      ...
      {hasGroups && (
        <>
          ...
          {(hiddenCount > 0 || (showAll && orderedKeys.length > VISIBLE_CAP)) &&
            !normalizedQuery && (
              <button
                type="button"
              ...
              >
                {showAll ? STRINGS.MSG_SHOW_LESS : STRINGS.MSG_SHOW_MORE(hiddenCount)}
              </button>
            )}
        </>
      )}

import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@metalbear/ui'
...
export function NamespaceFilter({ namespaces, value, onChange }: Props) {
  ...
  return (
    <Select
      value={selectValue}
      onValueChange={(v: string) => onChange(v === NAMESPACE_ALL_SENTINEL ? '' : v)}
    >
      <SelectTrigger
        id="ns-select"
        style={{
      ...
      <SelectContent>
        <SelectItem value={NAMESPACE_ALL_SENTINEL}>{STRINGS.MSG_ALL_NAMESPACES}</SelectItem>
        {distinct.map((ns) => (

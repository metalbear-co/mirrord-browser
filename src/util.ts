...
import { STRINGS, METALBEAR_EXTENSION_URL, CONFIG_HASH_PARAM } from './constants'
...
export function parseRules(rules: chrome.declarativeNetRequest.Rule[]): HeaderRule[] {
  const groups = new Map<string, { id: number; header: string; value: string; scopes: string[] }>()
  ...
  return Array.from(groups.values()).map((g) => ({
    ...
    header: g.header,
    value: g.value,
    scope: g.scopes.length === 0 ? STRINGS.MSG_ALL_URLS : g.scopes.join(', '),
  }))
}
...
export function aggregateSessions(sessions: OperatorSessionSummary[]): SessionGroupAggregate {
...
export function deriveInjectionHint(headerFilter: string | null | undefined): InjectionHint | null {
  if (!headerFilter) {
    return null
  }

...
import { MIRRORD_UI_DEFAULT_BACKEND, SESSION_NOTIFICATION_TYPE, STRINGS } from '../constants'
import {
  storageGet,
  storageSet,
  storageRemove,
...
export async function fetchOperatorSessions(
  ...
  if (isAuthFailureStatus(resp.status)) {
    const retry = await fetchImpl(`${url}?token=${encodeURIComponent(token)}`)
    if (retry.ok) {
      try {
...
export async function fetchContexts(
  ...
  if (!resp.ok) {
    const e = new Error(`mirrord ui responded ${resp.status} ${resp.statusText}`)
    ;(e as Error & { status?: number }).status = resp.status
    throw e
  }
...
function mapV2ToInternal(v2: OperatorSessionsV2Response): OperatorSessionsResponse {
...
export type PollResult =
  { ok: true; data: OperatorSessionsResponse } | { ok: false; status?: number; error: string }
...
export function buildWsUrl(backend: string, token: string): string {
  ...
  if (backend.startsWith('https://')) {
    ...
    throw new Error(`mirrord ui backend must be http:// or https://, got ${backend}`)
  }
...
function hasWatchedChange(changes: Record<string, chrome.storage.StorageChange>): boolean {
  return WATCHED_STORAGE_KEYS.some((key) => key in changes)
}

function groupByKey(sessions: OperatorSessionSummary[]): Record<string, OperatorSessionSummary[]> {
...
export function useMirrordUi() {
  ...
  const [sessions, setSessions] = useState<OperatorSessionsResponse | null>(null)
  ...
  const [selectedContext, setSelectedContextState] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const loadFromStorage = async () => {
      ...
      setBackend((stored[STORAGE_KEYS.MIRRORD_UI_BACKEND] as string | undefined) ?? null)
      setToken((stored[STORAGE_KEYS.MIRRORD_UI_TOKEN] as string | undefined) ?? null)
      setJoinState({
        joinedKey: (stored[STORAGE_KEYS.JOINED_KEY] as string | undefined) ?? null,
        joinedSessionName: (stored[STORAGE_KEYS.JOINED_SESSION_NAME] as string | undefined) ?? null,
      })
      const header = (stored[STORAGE_KEYS.JOINED_HEADER] as string | undefined) ?? null
      const value = (stored[STORAGE_KEYS.JOINED_VALUE] as string | undefined) ?? null
      ...
      setScopePatternsState(
        Array.isArray(persisted)
          ? (persisted as string[]).filter((p) => typeof p === 'string')
          : [],
      )
      setSelectedContextState((stored[STORAGE_KEYS.SELECTED_CONTEXT] as string | undefined) ?? null)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const refresh = () => {
      void runPoll(backend, token, v2Available, effectiveContext).then((result) => {
        if (cancelled) {
          return
        }
        if (result.ok) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      setSessions((current) => (current ? applyNotification(current, msg) : current))
    }
  ...
  const groupedFiltered = useMemo<Record<string, OperatorSessionSummary[]>>(() => {
    if (!sessions) {
      return {}
    }
  ...
  const removeScopePattern = useCallback(
    (pattern: string) => applyScopePatterns(scopePatterns.filter((p) => p !== pattern)),
    [scopePatterns, applyScopePatterns],
  )

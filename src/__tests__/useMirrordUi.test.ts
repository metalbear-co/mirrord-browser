...
import { useMirrordUi, fetchContexts, fetchOperatorSessionsV2 } from '../hooks/useMirrordUi'
...
let storageListeners: ((changes: Record<string, chrome.storage.StorageChange>) => void)[]
...
beforeEach(() => {
  ...
  ;(global as unknown as { chrome: unknown }).chrome = {
    ...
    storage: {
      local: {
        get: jest.fn((_keys: unknown, cb: (items: Record<string, unknown>) => void) => cb(storage)),
      ...
      onChanged: {
        addListener: jest.fn((l: (changes: Record<string, chrome.storage.StorageChange>) => void) =>
          storageListeners.push(l),
        ),
        removeListener: jest.fn(
          (l: (changes: Record<string, chrome.storage.StorageChange>) => void) => {
            storageListeners = storageListeners.filter((x) => x !== l)
          },
        ),
      },
    },
    declarativeNetRequest: {
      ...
      updateDynamicRules: jest.fn((_args: unknown, cb?: () => void) => cb?.()),
  ...
  global.fetch = jest.fn((url: RequestInfo | URL) => {
    if (urlToString(url).includes('/api/operator-sessions')) {
      return Promise.resolve(makeResponse(JSON.stringify(sampleResponse)))
    }
...
test('fetches sessions on mount when backend is configured', async () => {
  ...
  await waitFor(() => expect(result.current.sessions?.sessions.length).toBe(3))
...
test('clears authFailed once a freshly stored token is accepted', async () => {
  global.fetch = jest.fn((url: RequestInfo | URL, init?: RequestInit) => {
    const u = urlToString(url)
    const authToken = (init?.headers as Record<string, string> | undefined)?.['x-auth-token']
    if (u.includes('/health')) {
      return Promise.resolve({
        ok: true,
        status: 200,
...
test('session share uses the operator HTTP filter when it can derive a header', async () => {
  ...
  ;(global.fetch as jest.Mock).mockImplementation((url: RequestInfo | URL) => {
    ...
    if (urlToString(url).includes('/api/operator-sessions')) {
      return Promise.resolve(makeResponse(JSON.stringify(responseWithFilter)))
    }
    if (urlToString(url).includes('/health')) {
      return Promise.resolve(makeResponse('ok'))
    }
...
describe('v2 API', () => {
  ...
  test('fetchContexts parses the context list on success', async () => {
    ...
    expect(new Headers(f.mock.calls[0]?.[1]?.headers).get('x-auth-token')).toBe('t')
  })
  ...
  test('fetchOperatorSessionsV2 sends the selected context as a query param', async () => {
    ...
    expect(new Headers(f.mock.calls[0]?.[1]?.headers).get('x-auth-token')).toBe('tok')
  })
})

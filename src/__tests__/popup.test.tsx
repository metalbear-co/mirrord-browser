...
jest.mock('../headerObservation', () => {
  const actual = jest.requireActual('../headerObservation') as unknown as Record<string, unknown>
  return {
    ...actual,
    armCanary: jest.fn(),
...
describe('Popup', () => {
  ...
  it('shows active rule preview when rules exist', async () => {
    ...
    mockGetDynamicRules.mockImplementation(
      (cb: (rules: chrome.declarativeNetRequest.Rule[]) => void) => cb(rules),
    )
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('X-MIRRORD-USER: testuser')).toBeInTheDocument()
  ...
  it('renders scoped header rules', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('X-API-KEY: secret123')).toBeInTheDocument()
  ...
  it('resets to defaults when reset button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockStorageRemove).toHaveBeenCalledWith(['override'], expect.any(Function))
    })
  })
  ...
  it('shows inline error when saving with empty header name', async () => {
    ...
    await waitFor(() => {
      ...
      expect(errorEl).toHaveTextContent('Header name and value are required')
    })
  })
  ...
  it('renders share button', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Share configuration')).toBeInTheDocument()
    })
  })

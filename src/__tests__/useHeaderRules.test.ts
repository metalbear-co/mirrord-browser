...
const mockEmitUserBlocked = jest.fn();
const mockEmitUserSucceeded = jest.fn();
jest.mock('../analytics', () => ({
    capture: (...args: unknown[]) => mockCapture(...args),
    emitUserBlocked: (...args: unknown[]) => mockEmitUserBlocked(...args),
    emitUserSucceeded: (...args: unknown[]) => mockEmitUserSucceeded(...args),
}));
...
describe('useHeaderRules analytics', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockEmitUserBlocked.mockClear();
        mockEmitUserSucceeded.mockClear();
        (
    ...
    describe('emitUserBlocked typed reasons', () => {
        it('emits header_rule_save_failed on activate failure', async () => {
            mockGetDynamicRules.mockImplementation(
                (cb: (rules: chrome.declarativeNetRequest.Rule[]) => void) =>
                    cb([])
            );
            ...
            mockUpdateDynamicRules.mockImplementation(
                (_opts: unknown, cb: () => void) => {
                    ...
                    ).lastError = { message: 'activate failed' };
                    cb();
                    (
                        chrome.runtime as {
                            lastError: chrome.runtime.LastError | null;
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_save_failed',
                'user_action',
                expect.objectContaining({ error: 'activate failed' })
            );
        });

        it('emits header_rule_remove_failed on remove failure', async () => {
            const activeRule: chrome.declarativeNetRequest.Rule = {
                id: 1,
                priority: 1,
                action: {
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_remove_failed',
                'user_action',
                expect.objectContaining({ error: 'remove failed' })
            );
        });

        it('emits header_rule_save_failed on save update_rules failure', async () => {
            const activeRule: chrome.declarativeNetRequest.Rule = {
                id: 1,
                priority: 1,
                action: {
            ...
            mockUpdateDynamicRules.mockImplementation(
                (_opts: unknown, cb: () => void) => {
                    ...
                    ).lastError = { message: 'dnr save failed' };
                    cb();
                    (
                        chrome.runtime as {
                            lastError: chrome.runtime.LastError | null;
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_save_failed',
                'user_action',
                expect.objectContaining({ error: 'dnr save failed' })
            );
        });

        it('emits header_rule_save_failed on save storage_write failure', async () => {
            mockStorageSet.mockImplementation(
                (_data: unknown, cb: () => void) => {
                    (
                        chrome.runtime as {
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_save_failed',
                'user_action',
                expect.objectContaining({ error: 'storage write failed' })
            );
        });

        it('emits header_rule_reset_failed on reset storage_remove failure', async () => {
            mockStorageGet.mockImplementation(
                (
                    _keys: string[],
                    cb: (result: Record<string, unknown>) => void
            ...
            mockStorageRemove.mockImplementation(
                (_keys: string[], cb: () => void) => {
                    ...
                    ).lastError = { message: 'storage remove failed' };
                    cb();
                    (
                        chrome.runtime as {
                            lastError: chrome.runtime.LastError | null;
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_reset_failed',
                'user_action',
                expect.objectContaining({ error: 'storage remove failed' })
            );
        });

        it('emits header_rule_reset_failed on reset update_rules failure', async () => {
            mockStorageGet.mockImplementation(
                (
                    _keys: string[],
                    cb: (result: Record<string, unknown>) => void
            ...
            mockUpdateDynamicRules.mockImplementation(
                (_opts: unknown, cb: () => void) => {
                    ...
                    ).lastError = { message: 'dnr reset failed' };
                    cb();
                    (
                        chrome.runtime as {
                            lastError: chrome.runtime.LastError | null;
            ...
            expect(mockEmitUserBlocked).toHaveBeenCalledWith(
                'header_rule_reset_failed',
                'user_action',
                expect.objectContaining({ error: 'dnr reset failed' })
            );
        });
    });

    describe('emitUserSucceeded typed reasons', () => {
        it('emits header_rule_activated on successful activate', async () => {
            mockGetDynamicRules.mockImplementation(
                (cb: (rules: chrome.declarativeNetRequest.Rule[]) => void) =>
                    cb([])
            );
            ...
            expect(mockEmitUserSucceeded).toHaveBeenCalledWith(
                'header_rule_activated',
                'user_action'
            );
        });

        it('emits header_rule_removed on successful remove', async () => {
            const activeRule: chrome.declarativeNetRequest.Rule = {
                id: 1,
                priority: 1,
                action: {
        ...
        it('emits header_rule_saved on successful save', async () => {
            mockStorageSet.mockImplementation(
                (_data: unknown, cb: () => void) => cb()
            );
        ...
        it('emits header_rule_reset on successful reset', async () => {
            mockStorageGet.mockImplementation(
                (
                    _keys: string[],
                    cb: (result: Record<string, unknown>) => void

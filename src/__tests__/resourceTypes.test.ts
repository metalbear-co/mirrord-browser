...
>
const mockStorageSet = jest.fn() as jest.Mock<void, [data: unknown, cb: () => void]>
const mockStorageRemove = jest.fn() as jest.Mock<void, [keys: unknown, cb: () => void]>
...
describe('useHeaderRules resource types', () => {
  ...
  it('creates rules with all resource types so headers apply to scripts, stylesheets, images, etc.', async () => {
    ...
    expect(addedRules[0]?.condition.resourceTypes).toEqual(ALL_RESOURCE_TYPES)
  })

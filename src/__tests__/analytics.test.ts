...
import { capture, captureBeacon, setOptOut, loadOptOutState, optOutReady } from '../analytics'
...
describe('analytics', () => {
  ...
  describe('capture', () => {
    it('sends event via fetch to PostHog capture endpoint', () => {
      ...
      expect(body.api_key).toBe('phc_wIZh92nyk4vu6HidiLFUzjW6piZlZszuWZZFBS7yHHe')
  ...
  describe('opt-out', () => {
    ...
    it('setOptOut(false) removes key from chrome.storage.local', async () => {
      await setOptOut(false)
      expect(mockChromeStorage.remove).toHaveBeenCalledWith('analytics_opt_out')
    })

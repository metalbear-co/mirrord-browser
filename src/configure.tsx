...
import { buildDnrRule, getDynamicRules, updateDynamicRules, storageGet, storageSet } from './util'
import { CONFIGURE_STATUS, STRINGS, type ConfigureStatusKind } from './constants'
...
async function storeBackendAndToken(backend: string, token: string): Promise<void> {
  await storageSet({
...
async function joinKey(key: string, backend: string, token: string): Promise<Status> {
    ...
    <>
      ...
      <p className="text-muted-foreground text-sm">
        {status.kind === CONFIGURE_STATUS.JOINED ? (
          ...
          <>
            {STRINGS.LABEL_KEY}{' '}
            <code className="bg-muted rounded px-1 py-0.5 font-mono">{status.key}</code> {copy.hint}
          </>
        ) : (
          copy.hint
        )}

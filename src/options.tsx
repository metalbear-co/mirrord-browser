...
export function Options() {
  ...
  useEffect(() => {
    void optOutReady.then(() =>
      chrome.storage.local.get(STORAGE_KEYS.ANALYTICS_OPT_OUT).then((result) => {
        setAnalyticsEnabled(result[STORAGE_KEYS.ANALYTICS_OPT_OUT] !== true)
        setLoaded(true)
      }),
    )
  }, [])
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-6 p-6">
      <div className="border-border flex items-center gap-2 border-b pb-3">
        <span className="text-meta text-muted-foreground font-medium">{STRINGS.LABEL_MIRRORD}</span>
        <span className="text-muted-foreground text-xs">{STRINGS.SETTINGS_TITLE}</span>
      </div>

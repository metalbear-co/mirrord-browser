...
import { decodeConfig, isRegex, parseHeader, promptForValidHeader } from './configCore'
...
export async function run(): Promise<AppliedState> {
  ...
  try {
    ...
    return {
      kind: 'error',
      error: err instanceof Error ? err.message : 'The config link is malformed.',
      input: payload,
    }
  }
...
function AppliedPage() {
  ...
  return (
    <div className="mb-card">
      {state.kind === 'loading' && <p className="mb-text">{STRINGS.MSG_APPLYING_CONFIG}</p>}

      {state.kind === 'done' && state.joinedKey && (
        <>
          <span className="mb-eyebrow mb-eyebrow--success">{STRINGS.MSG_JOINED_TAG}</span>
          <h1 className="mb-title">{STRINGS.MSG_JOINED_LIVE_SESSION}</h1>
          <p className="mb-text">
            {STRINGS.MSG_ROUTING_TO_SESSION} <code className="mb-code">{state.joinedKey}</code>{' '}
      ...
      {state.kind === 'done' && !state.joinedKey && (
        <>
          <span className="mb-eyebrow mb-eyebrow--success">{STRINGS.MSG_CONFIGURED_TAG}</span>
          <h1 className="mb-title">{STRINGS.MSG_HEADER_CONFIGURED}</h1>
          <p className="mb-text">
            {STRINGS.MSG_JOINED_SESSION_HINT}{' '}
            <code className="mb-code">
              {state.header}: {state.value}
            </code>{' '}
            {state.scope ? `on requests matching ${state.scope}.` : 'on all requests.'}
          </p>
        </>
      )}
      ...
      {state.kind === 'error' && (
        <>
          <span className="mb-eyebrow mb-eyebrow--error">{STRINGS.MSG_COULDNT_APPLY_TAG}</span>
          <h1 className="mb-title">{STRINGS.MSG_COULDNT_APPLY_CONFIG}</h1>
          <p className="mb-text">{state.error}</p>
          {state.input && (
            <>
              <p className="mb-label">{STRINGS.MSG_INVALID_INPUT}</p>

...
export async function loadOptOutState(): Promise<void> {
  try {
    const result = await chrome.storage.local.get(STORAGE_KEYS.ANALYTICS_OPT_OUT)
 ...
 */
export function captureBeacon(event: string, properties?: Record<string, unknown>): void {
  if (optedOut) {
    return
  }
  try {

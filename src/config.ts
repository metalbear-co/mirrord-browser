...
import { isRegex, parseHeader, decodeConfig, promptForValidHeader } from './configCore'
...
export function storeDefaults(
  ...
  return storeHeaderConfig(STORAGE_KEYS.DEFAULTS, headerName, headerValue, scope)
}
...
export function storeOverride(
  ...
  return storeHeaderConfig(STORAGE_KEYS.OVERRIDE, headerName, headerValue, scope)
}
  ...
  return new Promise((resolve) => {
    ...
    chrome.storage.local.set({ [storageKey]: config }, () => {
      if (chrome.runtime.lastError) {
        console.error(`Failed to store ${storageKey}:`, chrome.runtime.lastError.message)
      }
      resolve()
    })
  })
...
function setHeaderRule(
  header: string,
  scope?: string,
  storageKey: typeof STORAGE_KEYS.DEFAULTS | typeof STORAGE_KEYS.OVERRIDE = STORAGE_KEYS.DEFAULTS,
): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    chrome.declarativeNetRequest.getDynamicRules((existingRules) => {
      chrome.declarativeNetRequest.updateDynamicRules(
        {
          removeRuleIds: rules.map(({ id }) => id).concat(existingRules.map((rule) => rule.id)),
          addRules: rules,
        },
        () => {
          if (chrome.runtime.lastError) {
            console.error('Failed to set header:', chrome.runtime.lastError.message)
            emitUserBlocked('configure_failed', 'user_action', {
              error: chrome.runtime.lastError.message ?? 'DNR update failed',
            })
            ...
            const storeConfig = storageKey === STORAGE_KEYS.OVERRIDE ? storeOverride : storeDefaults
            const clearSession =
              storageKey === STORAGE_KEYS.OVERRIDE ? clearJoinedSession() : Promise.resolve()
            void clearSession.then(() => storeConfig(key.trim(), value.trim(), scope)).then(resolve)
          }
        },
      )
    })
...
async function handleConfigLink(): Promise<void> {
  ...
  const storageKey =
    params.get('storage') === STORAGE_KEYS.OVERRIDE ? STORAGE_KEYS.OVERRIDE : STORAGE_KEYS.DEFAULTS

  if (!encoded) {
    alert('Configuration data missing, please make sure to copy the complete link.')
    return
  }

import { STORAGE_KEYS, type ThemePref, type StorageChange } from './types';
...
export async function saveTheme(pref: ThemePref): Promise<void> {
    try {
        await browser.storage.local.set({ [STORAGE_KEYS.THEME]: pref });
    } catch {}
}
...
export function initTheme(): () => void {
    ...
    const storageListener = browser.storage?.onChanged;

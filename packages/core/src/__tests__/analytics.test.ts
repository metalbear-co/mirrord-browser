...
// chrome.storage.local is backed by the shared webextension-polyfill mock (installed on
// the global `chrome`/`browser` by jest.setup.ts), so we assert against `chrome.storage.*`.
